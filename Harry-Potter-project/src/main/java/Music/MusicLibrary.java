package Music;

import java.io.File;

public class MusicLibrary {
    public static String backgroudMusic = new File("src/main/java/Music/Wide_Putin.wav").getAbsolutePath();
    public static String lezgongue = new File("src/main/java/Music/Lezgongue.wav").getAbsolutePath();
    public static String deathMusicAstronomia = new File("src/main/java/Music/Astronomia.wav").getAbsolutePath();
    public static String minecraftHit = new File("src/main/java/Music/minecraftDamage.wav").getAbsolutePath();
    public static String dodgeLoutre = new File("src/main/java/Music/EsquiveLoutre.wav").getAbsolutePath();
    public static String potionSlurpFortnite = new File("src/main/java/Music/PotionSlurpFortnite.wav").getAbsolutePath();
}
